...
//        when
        ...
        final var request = get("/categories/{id}", expectedId).with(ApiTest.CATEGORIES_JWT);
...
//        when
        final var request = get("/categories/{id}", expectedId.getValue()).with(ApiTest.CATEGORIES_JWT);
        final var response = this.mvc.perform(request)
                .andDo(print());
// then
...
//        given
        ...
        final var aUpdaterequest = new UpdateCategoryRequest(expectedName, expectedDescription, expectedIsActive);
//        when
        final var request = put("/categories/{id}", expectedId)
                .with(ApiTest.CATEGORIES_JWT)
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON)
                .content(mapper.writeValueAsString(aUpdaterequest));
...
//        then
    ...
    @Test
    public void givenAValidCommandWithInvalidID_whenCallsUpdateCategory_shouldReturnNotFoundException() throws Exception {
//        given
        final var expectedId = "123";
        final var expectedName = "Filmes";
        final var expectedDescription = "A categoria msis assistida";
        ...
        when(updateCategoryUseCase.execute(any())).thenThrow(NotFoundException.with(Category.class, CategoryID.from("123")));
...
//        given
        ...
        when(updateCategoryUseCase.execute(any())).thenReturn(Left(Notification.create(new Error("'name' should not be null"))));
...
//        when
        ...
        final var request = delete("/categories/{id}", expectedId).with(ApiTest.CATEGORIES_JWT);
...
//        then
    ...
    public void givenAValidParams_whenCallsListCategories_shouldReturnCategories() throws Exception {
        //        given
        final var aCategory = Category.newCategory("Movies", "tsetase asdf asd", true);
